...
void odom_vel_callback(const geometry_msgs::Pose2D &robot_vel)       // odom. vel. info.
{
    vx = robot_vel.x;
    vy = robot_vel.y;
    vth = robot_vel.theta;
...
void odom_pos_callback(const geometry_msgs::Pose2D &robot_pos)       // odom. pos. info.
{
    x = robot_pos.x;
    y = robot_pos.y;
    th = robot_pos.theta;
...
void odom_time_callback(const std_msgs::Time &robot_time)             // odom. time stamp
{
    current_time = robot_time;
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber odom_vel = n.subscribe("robot_vel", 1000, odom_vel_callback);
  ros::Subscriber odom_pos = n.subscribe("robot_pos", 1000, odom_pos_callback);
  ros::Subscriber odom_time = n.subscribe("robot_time", 1000, odom_time_callback);  
